...
- [IMPLEMENTED] `BankerSqlStore` owns typed relational `cd_ledger_items` and `cd_stock_items`, plus two-row `cd_banker_state` metadata. One item per row, no file paths/chunks/JSON item payloads. Indexed item templates, transaction, case-normalized banker/location, unique ledger ID and donor/time. Batched changed-row upserts/removals reuse the existing cross-domain custody transaction. DateTime ticks/kind preserve precision/nulls alongside readable DATETIME(6). Stock keys derive from physical addresses; duplicate IDs/addresses are rejected.
- [UPGRADE] Host performs additive table creation after its exclusive lease and before AO startup, then atomically imports both existing SQL sources with canonical field-by-field/ordering readback comparison. Upgrade marker/schema v2 commit with the rows. Source documents and original immutable archives remain preserved; completed upgrade never reimports them. Missing upgraded metadata fails rather than restoring stale state. v1 host upgrades automatically; old binaries reject v2. No DataMigration rerun/reset. CREATE permission is needed once.
- [CALL SITES] All live ledger readers/writers, census/recovery paths, Manager item/GET/lost-found queries, withdrawal validation and catalog template evidence now use named relational APIs. All stock saves/readers use relational APIs in shared RuntimeState and storage transactions. Removed obsolete ledger/stock path getters. Worker recovery queries only that character's rows and resolves routing in one catalog batch, avoiding hundreds of per-item SQL writer acquisitions. All seven projects explicitly include the new shared source.
- [OLD EVENTS] Removed automatic background MigrateLegacyHistory scheduling. Existing live donation/withdrawal accounting remains; retained legacy routine is not invoked automatically. Other data types remain on the existing backend pending separate work; no claim of full normalization.
- [EVIDENCE / LIMIT] Owner log shows early worker scans delayed while176s background history ran and recurring operational heartbeats stale80-100s. No fabricated live performance outcome. Static review covers migration atomicity/retry/version boundary, field mapping, nullable/date preservation, index predicates and all replaced call sites; lexical C# delimiters, project XML and diff checks. No assistant compilation, tests, SQL connection or AO run, per owner boundary. Restart matching build against existing SQL.

## Session 197 — SQL readiness snapshots and heartbeat load

- [OWNER LIVE] Session196 build behavior is present: Central deferred ledger initialization returned in about1s and background history completed in179.4s. All nine bankers reached BANKER READY by15:32:35. Support/Control/Extermination reconnected and resumed without audit, but final status still rejected five bankers, including two that never disconnected. Prior explanation attributing all unavailability to pending admission is superseded. The log lacks the rejected heartbeat/connection records; it does not prove the exact rejection or disconnect cause.
- [FIX] Small SQL documents (up to1MiB) now read metadata and exact ordered chunks in one SELECT, using existing unique path and document-offset indexes, instead of opening a separate repeatable-read streaming transaction per tiny record. Existence checks also use one statement. Enclosing transactions retain read-your-writes; independent logs retain independent reads. Missing/empty/oversized documents are distinct; large records keep streaming; chunk completeness/offset validation and SQL failure shutdown remain. No mutable-state cache, schema change or file fallback added.
- [FIX] BankerReadiness reads roster once to select keys, then all roster/cycle/ready/blocked/presence/operational records in one bounded statement snapshot. It rechecks the roster, connection, generation, Central dependency and unchanged10s monotonic freshness. This removes many sequential per-banker transactions and inconsistent cross-record reads. Manager status/console now includes concrete missing/stale/mismatched/held reasons and heartbeat age.
- [LOAD] Presence and operational heartbeat writes are paced at2s instead of500ms, with presence reset on connection change. Both still originate from AO actor callbacks; no background heartbeat, freshness extension, automatic audit or loss of SQL custody atomicity.
- [VALIDATION] Focused source, SQL statement/index, missing/empty/large record, transaction and readiness race review; lexical delimiter and git diff checks. No assistant build, test suite, SQL connection or live AO run, per owner boundary. Actual latency and remaining disconnect cause require owner runtime evidence. Deploy matching assemblies using existing migrated SQL; no reset or reimport.

## Session 196 — census SQL latency and completed startup reconnect

- [OWNER LIVE] Latest run survived SQL startup but Central spent164s in deferred ledger initialization, first worker scans then completed without bag failures, applications lagged20-30s, and three workers disconnected before admission. They reconnected but were held. The source10s readiness freshness threshold made slow updates unavailable; the log does not establish the cause of network disconnections.
- [PERFORMANCE] PhysicalLedgerReconciliation validates a full census then resolves all observed low/high AOIDs through one GetRulesFor SQL snapshot, preserving low-ID precedence, high-ID fallback, personal/CRU exclusions and central fallback. Removes per-item metadata round trips on worker validation and Central apply. Legacy departure history backfill runs once per domain on a background task, with atomic per-departure deduplication/index/history from session195; mandatory current-ledger initialization still completes synchronously. Background task has no AO operations and reports timing/failure.
- [RECOVERY] Before retiring a disconnected worker's admission, preserve its complete accepted scan and captured top-level inventory only if operations never started and no fault/shared-bag uncertainty exists. On fresh InPlay plus bank state, require same released cycle and identical inventory/bag identities/slots/templates/quantities. Rebind only connection ownership, retaining the identical run/result; Central reserves and idempotently applies the existing bundle. Never issue another audit. Changed evidence, partial scans or previously operational workers retain conservative recovery/admin policy.
- [SDK GUARD] A disconnect can clear SDK objects while a queued update unwinds. InPlay probe tolerates absent SDK session; offline-only null callbacks defer to disconnect invalidation. Other census exceptions now include stack traces. Readiness freshness remains10s; no fake background operational heartbeat or deadline extension.
- [VALIDATION / LIMIT] Focused source/interleaving and routing equivalence review, complete-run replay marker checks, lexical delimiters and diff checks. No assistant compilation, tests, SQL connection or live AO run. Deploy matching build and restart with existing migrated SQL. No reset or reimport needed; no confirmed performance numbers for this patch yet.
...
- [BACKEND] MySQL 8.0+ (not MariaDB), InnoDB, pinned MySqlConnector 2.3.7. Bootstrap MySql.Host/User/Password/Database, optional Port/SslMode. Schema v1 contains metadata/gates, logical namespaces, live documents and bounded raw chunks, indexed event lines, migration runs/files and immutable source chunks. JSON state has a queryable projection; raw bytes remain authoritative. Schema/index definitions and executable inspection commands are documented. Server max_allowed_packet must be at least32MiB for bounded16MiB JSON projections; the424MiB catalogue streams without whole-file materialization.
- [MIGRATION] With all old bots stopped, DataMigration inventories/hash-stages every original file recursively, including unknown, malformed, binary, hidden, empty, backup and temporary content. Each file's current document and immutable original archive commit together; full SQL readback and a final source recheck precede sealing. Verified source cleanup is restartable and removes the data directory. Sealed reruns verify/archive-cleanup only and cannot overwrite evolved runtime state. Changed pre-seal input must match its original inventory or use a fresh database; it is never silently omitted. Empty installations require --empty. Windows uses same-handle verified deletion; Mono uses double-hash deletion under the owner's offline/no-writers premise.
- [STARTUP / FAILURE] Schema, sealed import, completed cleanup and absence of a physical data path are required before AO login. A database-scoped exclusive lease excludes another updated host or importer. Runtime database failures terminate the entire process from any AppDomain; the lease heartbeat also detects disconnection/loss. No offline queue, file log, crash dump or automatic SQL operation replay is introduced. The importer and legacy executable cannot coordinate through SQL: stop the old executable first.
- [ATOMICITY] Cross-AppDomain SQL writer serialization and nested database transactions preserve accounting/history/storage/stock/withdrawal/queue changes. Custody receipts and initial-audit-used tokens retain create-only semantics. A rejected nested persistence operation marks the transaction rollback-only. Business locks never enclose physical AO actions. The no-automatic-audit policy and public service limits remain unchanged.
- [LOGGING / DEBUGGING] Full host diagnostics and source service events append in independent synchronous SQL transactions, avoiding cross-AppDomain logger/state lock deadlock and retaining evidence across state rollback. Source events are durable before bounded optional relay; duplicate canonical/source copies share their event Id. No local rotation discards diagnostics. Diagnostic tails are bounded and reports identify mysql: keys. Time/actor/event/transaction/problem indexes, JSON stock/withdrawal examples and actual SHOW CREATE/SHOW INDEX output are provided in docs/mysql-diagnostics.sql and DataMigration --schema.
- [RELATED FIXES] Fixed stale tell claims recreating completed jobs, stale sender acknowledgements, sequence/publication gaps, buffer ban mutation rollback, and corruption fallback that could silently reset authority. Flipper no longer substitutes a toggle after an unreadable request. Catalog bulk lookup uses one coherent snapshot; SQL-first cache locking avoids inversion, and large catalogue construction never holds the global SQL writer. Baseline latest/archive writes now remain consistent after retry.
- [VALIDATION / LIMIT] Independent focused source reviews, runtime disk-I/O audit, project XML/reference/source inclusion checks, C# lexical delimiter and whitespace checks. No assistant compilation, automated test suite, SQL connection/import or AO/live load test. No actual owner data has been migrated here: no SQL endpoint or source folder was supplied. Owner builds, supplies connection settings and runs DataMigration while bots are down; published implementation is resolved independently of that deployment, per owner policy. Credentials/private source data were not added to Git; no in-game changelog wording invented.

## Session 191 — public multiuser review and concurrency fixes

- [IMPLEMENTED] Public Manager work admission is bounded:24 background jobs total/3 per canonical member; public command burst6, refill1 per2s; shared burst64/refill10 per second; private overload notices at most once per10s. Owner's public commands are included. Only authenticated administrative controls bypass saturation. Sender identity is preserved across org/tell/guest tasks; new PublicLoad source explicitly included in csproj. Stock rendering no longer blocks chat processing.
- [IMPLEMENTED] Public input pauses when pending tells reach256 and resumes below128, with one-second sampling and transition logs. Existing transaction/service tells are preserved; severe-backlog input is intentionally not queued and does not add rejection-message backlog. Direct buffer input has burst4/refill1 per2s and the same delivery backpressure. This is admission control, not a hard maximum on output from already accepted work.
- [FIXED] Manager stock selection/admission serialized to avoid competing tasks selecting the same copy before reservation. Existing named-mutex admission/revision checks and collector ownership retained. Four pickup orders/three items each remain physical staging limits; not four simultaneous trades and not a promise to admit ten orders.
- [FIXED] Banker IPC uses atomic queued/claimed/cancelled ownership: timeout cannot cancel started mutations and falsely report rejected admission. Started requests report pending. Queue bounded64/proposals processed32 per tick. CRU replies distinguish pending from not queued.
- [FIXED] New trade cannot overwrite a previous pending receipt or claim pickup rows while it remains unresolved. Late Declined preserves finished and cancellation receipt verification. Donation bridge checks actual actor ownership. Nonempty donations have a two-minute monotonic total deadline and15s donor retry backoff on timeout, preventing endless edits from occupying Central forever. Relog timing rules unchanged.
- [FIXED] Buffer queue admission locked and duplicate-checked atomically;32 pending/current entries per character,256 per buffer. Duplicate does not drop later distinct requests. One cast exception removes only the failing current request instead of clearing all users' queue. Explicit administrative clear preserved.
- [REVIEW] Existing withdrawal mutex/revalidation/revision/collector checks, AO-thread ownership, and raid lock/owner admission inspected. No automatic audit paths restored. Report and owner multiuser scenarios:docs/MULTIUSER_REVIEW.md.
- [VALIDATION / LIMIT] Static source/call-path/interleaving review, active-source delimiter/conditional checks, project XML and new Compile inclusion, git diff --check. Owner builds/live tests; no assistant compilation or automated/live test. No honest ten-user throughput/sign-off claim from static inspection. Physical trade slot fairness against coordinated abuse, larger order capacity and live AO behavior remain practical limits, not promises.

## Session 190 — automatic recovery audits disabled

- [OWNER DIRECTION / IMPLEMENTED] Relog is legitimate recovery for clientless bankers. Manager is excluded from this recovery change. One initial startup audit per banker per host remains allowed; after that physical audits are exceptional, explicit administrator work, not SC+ authority and not routine failure/reconnect/periodic behavior. Do not resurrect automatic audits.
- [EXECUTION BOUNDARY] BagAuditAgent accepts only the startup gate's currently issued run once, with a flushed per-character initial-audit-used marker under the host generation. Consumption precedes bag enumeration/movement and survives domain/plugin reload in that process. Failed or interrupted attempts do not replenish the allowance. Normal enrollment commands are disabled. Separate existing console-admin CityDwellers.exe bankers-bagaudit remains available; no new chat/SC+ audit command was introduced.
- [DISABLED, SOURCE RETAINED] Operational local, paired dispatch, withdrawal, failed-dispatch and storage-recovery scan triggers/workers are commented out or excluded with #if false. Their IPC requests are explicitly denied. Automatic shared-bag evacuation is no longer driven by the startup gate. Late first-startup admission is permitted only for an unused initial scan. Failed scans do not retry. A host gets one coordinated startup cycle; local faults cannot replace it and re-audit peers.
- [LOCAL RECOVERY] Block now records the affected banker's fault and queues its own relog, never publishing roster recovery requests. Active trade prevents relog both at scheduling and disconnect boundaries. Intentional relog uses the existing SDK network session and schedules reconnect with zero added delay, restoring the configured network-failure retry delay afterward. No competing client/update loop, no Manager restart/relog. Banker bank reopening and idle-reconnect handoff no longer impose fixed world-settling waits; they wait for in-play plus a fresh bank snapshot.
- [RESUME / LIMIT] An idle banker with matching fresh top-level inventory/bank evidence rebinds only its own connection and resumes without opening bags. A repeated unresolved fault is reported rather than relogged endlessly. Pending operations, changed evidence, failed initial scans or failed reconnect interfaces stay held on that banker for administrator review; this change does not infer container contents, clear uncertain receipts, replay partial accounting or promise every fault is fixed by relog. Healthy peers keep their cycle and tokens. Public request readiness still requires Central because requests depend on Central; that does not audit/relog peers.
- [ROSTER ISOLATION] Legacy recovery-request files no longer invalidate every readiness result. Completed initial snapshots can release healthy members while an offline member remains unavailable by its own presence/ready checks. A pre-scan relog rebinds that member without creating another startup cycle. A failed/absent Central during initial collection still needs administrator attention; no replacement roster scan is allowed.
- [VALIDATION] Focused static entry-point/call-path review, conditional-compilation/delimiter checks, SDK reconnect scheduling/API source inspection and git diff --check. No assistant compilation, test suite or AO connection per owner boundary. Owner rebuild/restart deploys the change. Source only; no private SDK/logs/credentials committed. No owner-authored in-game changelog entry invented.
...
- [IMPLEMENTED] Added standalone .NET Framework4.8 x86 LoginTry project, output logintry.exe in release/debug, integrated in solution and as build-only dependency of CityDwellers. Local Directory.Build.props/targets isolate it from production source injection. Pinned Clientless1.0.16/SDK1.0.91, reusable pinned GameData restore; no private SDK source/binaries committed.
- [EXPERIMENT] logintry <aoaccount> <aopass> <aochar>: nine sequential fresh-client login/disconnect cycles on Rubi-Ka. One serial manual update driver, built-in SDK loop and chat disabled; SDK asynchronous network I/O remains. No plugins, audits, reconnect cooldown or additional in-world dwell. AutoReconnect=false and ReconnectDelay=0 remove the SDK's default30-second retry schedule. Disconnect immediately after CharacterInPlay processing returns, unload the domain fully, then begin the next attempt without a sleep. One-millisecond pump pacing is not an inter-attempt delay.
- [MEASUREMENT] UTC event times and monotonic milliseconds for login/auth/character/full-character/in-play, disconnect call/event, domain unload, setup and prior-disconnect-to-next-login gap. Log and console show each outcome and min/median/mean/max successful-login duration. Nine attempts include failures;120-second pump timeout advances failed attempts, Ctrl+C cancels, unload failure aborts to avoid overlapping clients. Synchronous SDK calls may exceed the pump timeout. LoginError enum is logged; raw SDK text, credentials and argument strings are not. Command-line credentials are not persisted.
- [LIMIT] Local disconnect/unload is not a server logout acknowledgement. Next successful login tests acceptance. Fresh AppDomain startup cost is measured separately; this experiment measures fresh-client cycles, not reuse of an existing network session. No policy conclusion or server cooldown claim is warranted before owner's results.
- [VALIDATION] Supplied SDK source plus deployed DLL metadata verify UseCurrentDomain's seven arguments, internal Update(double), Disconnect and reconnect configuration. Focused source/lifecycle/credential-log/one-client review, project XML and solution mappings, git diff --check. No compilation, test suites or AO connection performed. Normal banker/audit behavior remains unchanged. Owner next builds Release, runs logintry with their own account/character, and shares logintry-*.log.
...
## Session 185 — audited whole-host shutdown and org controller review

- [OWNER REQUEST] Check the prior organization blob up/down implementation and add one-command #shutdown for Squad Commander+ and administrators, with a durable actor log because shutdown is terminal.
- [VERIFIED SOURCE / OWNER STATUS] Controller is present. Owner status reports current5850/default5200, proven-safe budget5300, max exact-confirmed physical payload4956B, no failed probe, last unconfirmed644B, zero retries.5850 is an exploratory page budget, not a proven transport limit. Only near-budget org blob replies train growth; tell replies and small messages do not. Qualifying success grows roughly10%; eligible unproven frontier misses wait15s then shrink/rebuild/retry with at least3s grace. Small/previously-dominated misses do not retry. Retries do not immediately grow. Known safe/failed bounds converge in50-byte resolution, capped32768. This is not unlimited background probing or guaranteed recovery from every org failure. No org-controller code changed.
- [IMPLEMENTED] #shutdown and bare tell shutdown use existing admin identity rules or existing SC/General/President officer authority with reliable-alt checks/targeted lookup and rank fallback. Existing source membership and ban gates remain. One word, no confirmation prompt. Extra arguments do not stop anything. Duplicate accepted requests are suppressed. #restart remains admin-only and Manager-only; its outdated help now says so.
- [AUDIT] New append-only data/citydwellers-shutdown-audit.jsonl records authorized-request and host-accepted, request ID, actor, sender ID, authority character/rank, channel, UTC time and build. Each write is flushed to disk before request publication/host acceptance. Audit failure prevents that step. Manager diagnostics and host log also name the actor. The request is atomically handed off through a flushed temporary file; known runtime filenames are registered. New startup discards stale request markers but retains audit history.
- [LIFECYCLE] Host checks shutdown before restart, signals all component stop handles and uses the existing bounded cleanup. Console then exits even if a foreground component remains. Windows service reports a normal zero-code stop from a separate thread before exit, preventing configured failure recovery from restarting this intentional stop, including after earlier component errors. Normal service auto-start on a later machine boot is unchanged. In-game acknowledgement is best effort; the durable audit is authoritative even if chat delivery fails.
- [HELP] Command index and shutdown help describe scope/access/manual restart. Shutdown syntax is plain text, not an executable help link. No owner changelog wording was supplied, so no in-game changelog was added.
- [VALIDATION] Focused static review of authorization, command routing, file handoff/audit ordering, duplicate/stale requests, service self-join/recovery and terminal exit; project XML parsing and git diff --check. No compilation, test suites or live AO actions under the owner boundary. Resolved on publication; owner rebuilds the unified host and Manager together and may report any concrete failure.

## Session 184 — org self-echo confidence and faster probing

- [OWNER LIVE EVIDENCE] Fresh session183 build: organization status produced exact echo confirmations at 4879/2098 bytes and 4887/2100 bytes; #items operative confirmed 4940/1161 bytes. Immediately afterward #online produced a 644-byte reply that was visibly present in Athen Paladins, but Apcmanager did not observe its own exact self-echo. Session183 therefore retried it twice and the org saw three identical [Online] replies. This proves absence of Manager self-echo is not, by itself, proof of non-delivery. The reason that particular delivered message was not echoed back to Manager remains unknown.
- [SUPERSEDES SESSION183 NEGATIVE-EVIDENCE RULE] Exact positive echo remains authoritative delivery evidence. A missing echo is now allowed to train size/backoff/retry only when the message is a near-current-budget blob probe, is above the largest physical payload already exact-confirmed in this runtime, and (once known) its page budget is above the proven-safe page budget. All other missing echoes are recorded as uncertainty only: no blob downshift, no retry, and therefore no org spam from a small false-negative such as 644 bytes.
- [ADAPTIVE PROBING] 5200 remains the owner-editable default. Open-ended discovery no longer crawls by50: a confirmed frontier page records its page budget as proven safe, then raises the next exploratory budget by roughly10%, rounded to50 bytes with a minimum250-byte step. Example:5200 ->5700; another qualifying success -> about6250. Once a failed upper page budget exists, exploration switches to midpoint narrowing between proven-safe and failed budgets in50-byte resolution; retries establish safe bounds but do not immediately launch another larger probe.
- [PERSISTENCE] citymanager-org-size.json is version4. V3 state preserves only current budget and positive echo bytes; its old LastFailedBytes is migrated as unconfirmed because v3 incorrectly treated every self-echo miss as a size failure. V4 persists current/default, proven-safe budget, failed probe budget, last/max confirmed bytes, last unconfirmed bytes and last frontier-failure bytes.
- [STATUS] #status Org output exposes current(default), proven-safe budget, failed probe budget, maximum exact-confirmed physical bytes, last unconfirmed bytes and retry queue count. This makes both aggressive exploration and later convergence visible.
- [PRESERVED] Genuine frontier misses still use the session183 logical-page rebuild/retry path with at least3 seconds between sends and late-exact-echo cancellation. Guest/tell budgets remain fixed. No command is re-executed to retry output.
- [VALIDATION] Focused current-master static review: old OrgBlobPageStep and _orgLastFailedBytes references absent; v3 migration/v4 save-load present; larger-confirmed-payload dominance, near-frontier gating,10% growth,50-byte quantization and midpoint narrowing present; CityManager and Presentation brace counts balanced. Base-to-head changes are confined to Manager org controller plus recovery state until documentation. No assistant compilation, tests or live AO run. Owner rebuilds/live-tests. No in-game changelog entry supplied or added.

## Session 183 — adaptive organization blob budget and retry

- [OWNER LIVE EVIDENCE] At 2026-09-20 04:35-04:36 +03:00, Apcmanager submitted #online organization replies of 650 and 713 UTF-8 bytes through AOSharp.Clientless.Client.SendOrgMessage with Stat.Clan=4736, but no exact echo was observed within 15 seconds and the owner independently saw no organization-chat reply. A successful SendOrgMessage call is therefore still only a send attempt; exact sender/channel/full-text echo remains the delivery proof.
- [OWNER DIRECTION] Keep the editable 5200 organization blob size as the default/baseline. Restore feedback around that baseline rather than replacing it with the old low-start binary-search experiment: current size may drift upward/downward in 50-byte steps from real delivery evidence, persist that learned current value, retry missing replies, and show current(default) in #status.
- [IMPLEMENTED] Organization pagination now uses a persistent current budget initialized from default 5200. data/citymanager-org-size.json version3 stores default/current size, last confirmed bytes, last failed bytes and update time; incompatible retired calibration formats reset to 5200 and are rewritten in the new format. Guest/tell budgets remain fixed at 8000/7200.
- [IMPLEMENTED] A first-attempt blob page whose actual transport size is within512 bytes of the current budget can raise the next budget by50 after exact echo confirmation. Retries never raise the budget. Missing exact echo backs the current budget to one50-byte step below the budget that actually failed, then rebuilds that logical blob page at the smaller current budget without re-running its command and queues retry pages at least3 seconds apart. Concurrent pages rendered at one old budget do not compound the same evidence. Plain replies retry unchanged. If a single AO markup row cannot be safely repaginated, the already-rendered page remains retryable instead of being dropped.
- [IMPLEMENTED] The3-second retry grace is meaningful: a late exact sender/channel/full-text echo cancels all still-queued replacements for that failed page, preventing a duplicate resend when delivery was merely delayed. Retry queues are runtime-only; learned budget/evidence persists. #status Org output now includes blob current(5200 default), last confirmed bytes, last failed bytes and queued-retry count.
- [VERIFIED LIVE / RELATED] The same owner run shows the hardened Bobsan parser working in runtime: ALTS ONLINE parsed mains=10, online-characters=15, pages=1, and a passive group for hyphenated main Splicer-2 completed with unique=21. This supplies the runtime verification that sessions177-179 were waiting for; the historical zero-group cause remains unexplained, but current guarded parsing is functioning on the observed format.
- [VALIDATION] Focused static call-site/signature, partial-class visibility, retry-ordering, exact-echo, persistence-state and brace-balance review. CityManager.cs and CityManager.Presentation.cs brace counts match; sole org send/direct-send/pending-echo call surfaces are internally consistent. No assistant compilation, test suite or live AO run under the owner build/test boundary. No owner-authored in-game changelog entry was supplied or added.
...
- [FIXED] Added shared CharacterNames lexical rule (letters, decimal digits, hyphens) matching existing char.IsLetterOrDigit-based membership/admin/ban/alt validation. All five alt/online/presence name regexes use the shared token; alt headings/rows and login/logout now recognize hyphenated names. Buffer configuration uses the same character class. Validators retain existing length bounds, error messages and case behavior; lexical validation is not an existence test.
- [FIXED] Banker health-file writer already retained hyphens, while Manager status/inventory and withdrawal recovery readers stripped them. Shared FileToken now keeps writer/readers aligned. Internal trade readiness markers preserve hyphens rather than collapsing distinct names; bag diagnostic filenames retain hyphens too. Ordinary existing unhyphenated banker paths are unchanged. Rebuild/restart unified runtime together.
- [OBSERVABILITY] Unrecognized Bobsan login/logout formatting warns and retains existing presence; it does not declare a character nonexistent. Session178 complete-main coverage guard remains in place. No name expansion from alt mapping alone implies online presence.
- [AUDIT SCOPE] Searched repository C# regex character classes, letter/digit validators, name sanitizers and hyphen replacement. Membership/admin/ban already accepted hyphens; unified their predicate. Remaining ASCII expressions concern hashes/HTML entities or decorative log coloring, not character admission or identity lookup. No changes to those unrelated formats. This is a source audit, not a claim every arbitrary future game naming/markup format is supported. Earlier Bobsan zero-group cause still requires owner runtime result.
- Validation: static regex group numbering/shared compile inclusion, filename producer/consumer and diff review; no compilation/test suite/live AO operations per owner boundary. Owner next rebuild all and test online plus alt lookup/presence announcements for a hyphenated character. No private samples or invented in-game changelog text published.
