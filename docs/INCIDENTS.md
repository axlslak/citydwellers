...
Manager automatically creates and refreshes problem reports in the SQL `incident-dumps/` namespace every 30 seconds (first pass after 15 seconds). This works with syslog disabled. A report is available while recovery is still pending; later records update the same report. No full runtime log is copied into it. Manager must be running for automatic rendering; source evidence remains in SQL while it is unavailable.

The SQL `transaction-traces/` namespace holds the structured evidence. Sources write records under a database transaction; Manager alone renders dumps. This uses the mandatory MySQL backend and does not depend on the optional bounded syslog IPC queue. Files are not transmitted automatically. The existing Manager authorization for dump remains in effect.
...
- Exports cap linked traces at 64 and events at 5,000, reporting truncation. Original SQL trace records remain available. Related history can make a report larger, but it never falls back to the big log.
...
SQL storage and exact queries are documented in [MySQL migration](MYSQL_MIGRATION.md) and `mysql-diagnostics.sql`. No incident report is written to a local data folder.
